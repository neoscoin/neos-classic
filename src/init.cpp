...
std::string HelpMessage()
{
    ...
    strUsage += "  -dbcache=<n>           " + _("Set database cache size in megabytes (default: 150)") + "\n";
    ...
    strUsage += "  -rpcsslciphers=<ciphers>                 " + _("Acceptable ciphers (default: TLSv1+HIGH:!SSLv3:!SSLv2:!aNULL:!eNULL:!AH:!3DES:@STRENGTH)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. NeosCoin is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("NeosCoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    size_t nTotalCache = GetArg("-dbcache", 150) << 20;
    if (nTotalCache < (1 << 22))
